...
- 实现提交：`21d99e9`；分支：`codex/hold-unrealized-pnl`。
- 基准新增 `unrealized_pnl` 和 `unrealized_return`，明确表示期末未卖出的简单持有浮盈亏；不包含未发生的退出费用和税费。
- 前端新增简单持有浮盈亏、当前策略期末盈亏及两者差额；历史 run 没有新字段时按 `final_asset - initial_cash` 兼容回退。
- 前端全量 Vitest：5 files / 28 tests passed；改动文件 ESLint 通过；生产构建通过。
- 生产依赖容器直接断言通过：基准浮盈亏等于期末资产减初始资金，浮盈亏率等于净收益率，完整交易和卖出次数均为 0。
- Edge Chromium 扩展通道实机验收通过：扩展可直接识别、读取和操作 Edge 中的 TradingAgents 页面。

真实非单边上涨样本：`000858.SZ` 五粮液，2025-01-01 至 2025-12-31，腾讯前复权，默认 10 万元。run_id `73745cc5ccc1406e91595134c5af32a1`，数据哈希 `d02a95f7f7c50d2daae21656c998dccabb3de9db7e2b597b0d9f1195db7d1281`。

| 方案 | 期末资产 | 期末盈亏 | 净收益率 | 最大回撤 | 总成本 |
|---|---:|---:|---:|---:|---:|
| A | 100,086.92 | +86.92 | +0.09% | -7.69% | 1,269.08 |
| B | 100,397.54 | +397.54 | +0.40% | -7.25% | 1,066.46 |
| C | 102,689.58 | +2,689.58 | +2.69% | -4.94% | 514.42 |
| D | 102,689.58 | +2,689.58 | +2.69% | -4.94% | 514.42 |
| 简单持有（浮亏） | 81,878.28 | -18,121.72 | -18.12% | -19.93% | 103.72 |

该样本确认新指标在震荡下行市场中能直观显示择时策略相对简单持有的风险规避效果；切换策略 A 与 C 时，对比差额分别动态更新为 -18,208.64 元和 -20,811.30 元。
